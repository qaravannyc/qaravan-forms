...
const CANON = {
  highlights: ["Люди и общение", "Новые знакомства", "Атмосфера", "Тема и содержание",
    "Ведущие и организация", "Место", "Еда и угощения", "Было полезно", "Другое"],
  discomfort: ["Неудобное время", "Сложно добираться", "Неудобное место",
    "Слишком долго", "Слишком коротко", "Слишком тяжело физически", "Не с кем было поговорить", "Слишком много людей",
...
export default async function handler(req, res) {
  ...
  const lines = [
    ...
    // вопросов «в стране / возраст» в форме нет — вечное «— | —» только
    // мозолило глаза; строка вернётся, если такие ответы вообще придут
    (b.stay || b.age) ? `В стране: ${b.stay || "—"} | Возраст: ${b.age || "—"}` : null,
    `Согласие на цитаты: ${b.consent || "—"}${b.name ? ` (подпись: ${b.name})` : ""}`,
    `Связь: ${b.contact_ok || "—"}${b.contact_name ? ` | Имя: ${b.contact_name}` : ""}${b.email ? ` | Почта: ${b.email}` : ""}${b.phone ? ` | Телефон: ${b.phone}` : ""}`,
    `Донат: ${b.donate || "—"}`,
