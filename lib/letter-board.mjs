...
export const C = {
  ...
  dob: "date_mm6vvbwa",            // Date of birth (older, staff-entered rows; the form asks for age instead)
  age: "numeric_mm6vn2bm",         // Age
  ...
  attEmail: "email_mm6vf63b",      // Attorney email
  ...
  translationFeedback: "long_text_mm6v6zxk", // Translation feedback ("Spotted a translation problem?" on uk/ka/uz/kk)
};
...
// The same row, read straight by item id — never through the search index.
// items_page_by_column_values reads a search index that lags behind writes by
// seconds; every autosave that read the row through it got the previous save
// back, saw «resume email not sent yet» and sent it again: 4–7 emails per
// person on Sep 14–15, 2026. The row must carry this intake id, or it is not
// this person's (a stale or forged id gets nothing).
export async function findByItemId(itemId, rid) {
  if (!/^\d{5,20}$/.test(String(itemId || ""))) return null;
  const d = await monday(
    `query ($ids: [ID!]) { items(ids: $ids) { id group { id } column_values(ids: ["${C.rid}", "${C.raw}"]) { id text } } }`,
    { ids: [String(itemId)] }
  );
  const it = d?.items?.[0];
  if (!it || (it.column_values?.find((c) => c.id === C.rid)?.text || "").trim() !== rid) return null;
  let raw = null;
  try { raw = JSON.parse(it.column_values?.find((c) => c.id === C.raw)?.text || "null"); } catch (e) { raw = null; }
  return { id: it.id, group: it.group?.id, raw: raw && typeof raw === "object" ? raw : null };
}
